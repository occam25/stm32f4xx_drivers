...
uint8_t msg[] = {"Message from STM32: Por el val de las estacas paso el Cid a medio dia, en su caballo Bavieca, oh que bien que parecia! el rey moro que lo supo, a recibirle salia. \"Bien vengas, buen Cid. Buena sea tu venida, que si vienes a por sueldo muy bueno te lo daria, que si vienes por mujer, darte he una hermana mia\" \"Que no quiero vuestro sueldo, ni de nadie lo querria, que no vengo por mujer, pues viva tengo la mia! vengo a que pagues las parias que debes a Castilla\"\r\n"};
uint32_t msg_len;
...
void I2C_ApplicationEventCallback(I2C_handle_t *pI2CHandle, I2C_events_t AppEv)
{
	...
	if(AppEv == I2C_EV_DATA_REQ){
		...
		if(cmd_code == I2C_READ_LEN_CMD){
			// send length to the master
			I2C_SlaveSendData(pI2CHandle->pI2Cx, (msg_len >> (cnt%4 * 8)) & 0xff);
			cnt++;
		}else if(cmd_code == I2C_READ_DATA_CMD){
			// Send data
		...
		if(cmd_code != I2C_READ_DATA_CMD)
			cmd_code = 0;

		// reset the cnt because it is end of transmission
		cnt = 0;

		// Slave concludes the transmission when the msg index reaches msg len
		if(msg_idx >= msg_len){
			msg_idx = 0;
			cmd_code = 0;
		}else if(msg_idx > 0)
