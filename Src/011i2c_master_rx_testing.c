...
int main(void)
{
	...
	while(1){
		...
		I2C_MasterSendData(&i2c_handle, &cmd, 1, ARDUINO_I2C_ADDR, I2C_ENABLE_SR);

		// 2. Read arduino's data len
		I2C_MasterReceiveData(&i2c_handle, &data_len, 1, ARDUINO_I2C_ADDR, I2C_ENABLE_SR);
		...
		I2C_MasterReceiveData(&i2c_handle, data, data_len, ARDUINO_I2C_ADDR, I2C_DISABLE_SR);
